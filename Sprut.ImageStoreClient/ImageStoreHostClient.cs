...
namespace Sprut.ImageStoreClient
{
    public class ImageStoreHostClient : IDisposable
    {
        ...
        {
            //_service=new ImageService.ServiceClient();
            var remoteAddress = new EndpointAddress("http://localhost:56937/Service.svc");
            _service = new ServiceClient(new BasicHttpBinding(), remoteAddress);
        }
