...
{ 
    public class CommandController : ApiController
    {
        ...
        public IEnumerable<string> Get()
        {
            ...
            return types.Select(t => t.Name + " (" + string.Join(", ",(t.GetConstructors().FirstOrDefault(c => c.GetParameters().Length > 0)?? t.GetConstructors().First()).GetParameters().Select(p=>p.Name)) + ")");
        }
        ...
        public async Task<string> Get(string command, string key)
        {
            ...
            try
            {
                string name = command.Split('(')[0].Trim();
                List<string> param = command.Contains("(") ? command.TrimEnd(')').Split('(')[1].Split(',').Select(s => s.Trim()).ToList() : null;
                var fullName = "YAShop.BusinessLogic.Bus.Commands." + name;
                var t = typeof(ICommand).Assembly.GetType(fullName );
                List<object> forContructor = new List<object>();

                var pp = (t.GetConstructors().FirstOrDefault(c => c.GetParameters().Length > 0) ?? t.GetConstructors().First()).GetParameters();
                if (param!=null)
                {
                    int i = 0;
                    foreach (var p in pp)
                ...
                else if (forContructor.Count ==2) cmd = (ICommand)Activator.CreateInstance(t,forContructor[0],forContructor[1]);
                else if (forContructor.Count ==3) cmd = (ICommand)Activator.CreateInstance(t,forContructor[0],forContructor[1],forContructor[2]);
                else if (forContructor.Count ==4) cmd = (ICommand)Activator.CreateInstance(t,forContructor[0],forContructor[1],forContructor[2],forContructor[3]);
                if (cmd!=null)
                {
